...
function concatenateTypeDefs(typeDefinitionsAry) {
  ...
  return (0, _lodash.uniq)(resolvedTypeDefinitions.map(function (x) {
    return x.trim();
  })).join('\n');
}
...
function buildSchemaFromTypeDefinitions(typeDefinitions) {
  ...
  return (0, _utilities.buildASTSchema)((0, _language.parse)(myDefinitions));
}
...
function addSchemaLevelResolveFunction(schema, fn) {
  ...
  var rootResolveFn = runAtMostOnce(fn);
...
function addTracingToResolvers(schema, tracer) {
  forEachField(schema, function (field, typeName, fieldName) {
    ...
    // eslint-disable-next-line no-param-reassign
    field.resolve = (0, _tracing.decorateWithTracer)(field.resolve, tracer, { functionType: 'resolve', functionName: functionName });
  });
}
